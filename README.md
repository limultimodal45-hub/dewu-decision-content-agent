...
当前版本新增四象限矩阵，用 `exposure` 作为横轴、`decision_value_score` 作为纵轴，把社区内容分成四类：

- 已验证样板内容：高曝光、高决策价值。
- 被埋没的决策内容：低曝光、高决策价值。
- 虚热内容 / 曝光资源错配：高曝光、低决策价值。
- 低优先级观察内容：低曝光、低决策价值。

矩阵不判断内容真假，也不直接决定删除或封禁内容；它用商品点击、详情页访问、转化、取消/退货等后续行为，辅助判断曝光资源是否分配给了真正帮助用户购买决策的内容。

## 内容激励博弈沙盘

当前版本新增“内容激励博弈沙盘：平台规则如何改变创作者供给与用户信任”。它放在四象限矩阵之后，用确定性规则模拟平台激励变化对内容供给结构的影响。

沙盘包含四类创作者策略：

- 真实体验型：真实体验、价格判断、对比测评等高决策信息内容。
- 模板化真实体验型：看起来具备真实体验语言，但可能更偏模板化套利的内容。
- 同质化好评型：偏“百搭、好看、闭眼入”的低决策密度内容。
- 穿搭种草型：偏氛围展示和浅层互动的内容。
...
- 创作者策略份额演化
- 平台与用户生态指标变化，包括用户信任指数、搜索成本指数、曝光错配率和平台综合收益
- 四象限结构变化

该模块不调用 GPT，不预测真实 GMV，也不能替代线上 A/B 实验。它用于面试或产品 Demo 中解释：当平台从浅层互动转向“商品点击、决策深度、转化、低退货”的行为验证激励时，内容供给为什么可能从同质化好评转向更有购买决策价值的内容。
